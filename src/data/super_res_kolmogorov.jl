...
function load_super_res_kolmogorov_data(
    ...
    low_res_data = reshape(low_res_data, 128, 128, 2, num_trajectories * num_steps);

    if normal_base_distribution
        return (;
            base = Distributions.Normal(0.0f0, 1.0f0),
            target = high_res_data,
            field_conditioning = low_res_data
        )
    end
...
function load_super_res_kolmogorov_data(config::Config.SuperResKolmogorovDataHyperparameters)
