...
function posterior_drift_term(x, x0, y, t)
    ...
    obs_interpolant = alpha(t) .* obs_operator(x0) .+ beta(t) .* y #.+ gamma(t) .* randn(Float32, size(x))
    ...
    # interpolant_cov = interpolant_cov .+ gamma(t).^2 .* Id
...
exact_posteior_dist = Dist.MvNormal(0.5f0 .* (base_sample .+ obs), 0.5f0 .* Id)
